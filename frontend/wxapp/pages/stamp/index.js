...
import { updateUserStamp } from '../../model/user'
const app = getApp()
...
Page({
  ...
  onStamp() {
    ...
    if (value) {
      wx.showModal({
        title: '提示',
        content: '如点击确认，盖章码将失效，无法使用盖章机器人盖章',
        success(res) {
          if (res.confirm) {
            updateUserStamp(value).then((data) => {
              wx.reLaunch({
                url: '/pages/tweet/index',
              })
            })
              .catch(err => {
                wx.showModal({
                  title: '提示',
                  content: err.message + '，如有疑问请联系微信客服',
                  showCancel: false,
                })
              })
          }
  ...
  async draw(data, color) {
    // 获取canvas以及context
    const canvas = await this.getQrContainer();
    const ctx = canvas.getContext('2d');
    // 二维码的颜色
    const colorDark = color ?? '#000';
