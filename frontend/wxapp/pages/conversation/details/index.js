...
Page({
  ...
  async onLoad(options) {
    const { participant, from, title } = options
    if (title == "系统消息" || title == "奖励消息") {
  ...
  onMessage(e) {
    const { user } = app.globalData
    const { participant, from, conversations } = this.data
    const content = e.detail.value
    const input = { participant, to: from, content }
