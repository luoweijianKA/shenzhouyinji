...
const GET_PASSPORTS = gql`
  query GetInactiveUserPassports($first: Int = 20, $after: ID, $last: Int = 20, $before: ID, $filter: UserPassportFilter) {
...
function ExchangeModal({ value, ...props } : { value?: Passport } & DialogProps) {
  ...
  useEffect(() => {
    if (open && value) {
      const { id, passport_code, camp, user_id, event_id } = value
      setValues({ ...values, id, passport_code, camp_id: camp?.id, user_id, event_id })
    }
  }, [open, value])
...
export default function PassportBind() {
  ...
  const passports: Passport[] = useMemo(() => {
    if (data) {
      const { totalCount, edges, pageInfo } = data.inactiveUserPassports
      setValues({
        ...values,
        totalCount,
                ...
                <TableRow
                  ...
                  <TableCell scope="row" sx={{ textAlign: "right" }}>
                     <Stack
                        ...
                        spacing={0.5}
                      >
                        <Button variant="text" onClick={handleUpdate(row)}>绑定护照</Button>
                      </Stack>
                  </TableCell>
                </TableRow>
              ))}
