...
const GET_ACCOUNTS = gql`
  query GetAccounts($first: Int = 20, $after: ID, $last: Int = 20, $before: ID, $search: String) {
    accounts(
      first: $first
      after: $after
      last: $last
...
export default function User() {
  ...
  const { data, loading, refetch } = useQuery(GET_ACCOUNTS, { variables: { first, after, last, before, search }, fetchPolicy: "no-cache" })
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValues({ ...values, after: undefined, before: undefined, page: 0, search })
  }
              ...
              <TableContainer component={Paper} sx={{ boxShadow: 'none' }}>
                ...
                {accounts.map((row) => (
                  <TableRow
                    ...
                    <TableCell scope="row" component="th">
                      <ListItem alignItems="flex-start" sx={{ p: 0 }}>
                        <ListItemAvatar sx={{ mt: 0, mr: 1 }}>
                          <Avatar alt={row.wechat_name} src={process.env.REACT_APP_RESOURCES_DOMAIN + row.wechat_avatar} sx={{ width: 56, height: 56 }} />
                        </ListItemAvatar>
                        <ListItemText
                          primary={row.wechat_name}
                          secondary={
                    ...
                    <TableCell scope="row" component="th">
                      <Typography variant='subtitle2'>{formattedDate(new Date(row.create_time * 1000))}</Typography>
                    </TableCell>
                    <TableCell scope="row" component="th">
                      <Typography variant='subtitle2'>{row.city}</Typography>
                    </TableCell>
                    <TableCell scope="row" sx={{ textAlign: "right" }}>
                      <Tooltip arrow title={"用户详情"}>
                        <IconButton href={`#/user/${row.id}`}>
