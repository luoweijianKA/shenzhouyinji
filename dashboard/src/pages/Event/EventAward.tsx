...
const GET_EVENT_AWARDS = gql`
  query GetEventAwards($first: Int = 20, $after: ID, $last: Int = 20, $before: ID, $eventId: ID!, $code: String) {
    eventAwards(
      first: $first
      after: $after
      last: $last
...
export default function EventAward() {
  ...
  useEffect(() => {
    if (!eventId && events && events.length > 0) {
      ...
      setValues({ ...values, eventId: events[0].id})
    }
  }, [events])
  ...
  useEffect(() => {
    if (eventId && eventId.length > 0) {
      fetch({ variables: { first, after, last, before, eventId, code } })
    }
  }, [fetch, first, after, last, before, eventId, code ])

  const [createEventAwards] = useMutation(CREATE_EVENT_AWARDS, {
    refetchQueries: [
      { query: GET_EVENT_AWARDS, variables: { first, after, last, before, eventId, code } },
      "GetEventAwards",
    ]
  })
  ...
  const handleImportComplete = (file: string) => {
    if (file && file.length > 0) {
      ...
      createEventAwards({ variables: { input: { eventId, file } }})
  ...
  const handleDelete = () => {
    ...
    if (input.length > 0) {
      setDeleteValues({ value: { id: '', name: '活动奖励' }, open: true })
    }
  }
