...
const SCENERY_SPOT_CATEGOTRY = `${process.env.REACT_APP_CATEGOTRY_SCENERY_SPOT}`
...
function useSave({ value, onCompleted } : { value?: Category, onCompleted?: (data: any) => void }) {
  const add = useMutation(ADD_CATEGORY, {
    ...
    refetchQueries: [
      { query: GET_CATEGORIES, variables: { parentId: SCENERY_SPOT_CATEGOTRY } },
      "GetCategories",
    ]
  })
...
function CategoryModal({ value, ...props } : { value?: Category } & DialogProps) {
  ...
  const handleOK = () => {
    ...
    const input = (!id && name.length > 0)
      ? {
        name,
        parent_id: SCENERY_SPOT_CATEGOTRY,
        has_subclass: false,
        status: 1,
        sort: displayOrder,
      }
...
export default function PlaceCategory() {
  ...
  const { data, loading } = useQuery(GET_CATEGORIES, { variables: { parentId: SCENERY_SPOT_CATEGOTRY }, fetchPolicy: "no-cache" })
  ...
  const [deleteCategory] = useMutation(UPDATE_CATEGORY, {
    refetchQueries: [
      { query: GET_CATEGORIES, variables: { parentId: SCENERY_SPOT_CATEGOTRY }, fetchPolicy: "no-cache" },
      "GetCategories",
    ],
  })
